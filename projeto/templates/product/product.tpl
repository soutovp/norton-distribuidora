{% set gallery_type = "click" %}
{# gallery_type allow types -> (click, zoom and zoom_inner) #}
{% set gallery_thumbs_number = 3 %}
...
<script defer src="{{asset("etalage/etalage-1.3.4-min.js","libs")}}" type="text/javascript"></script>
<script type="text/javascript">
	$(document).ready(function () {
let $_gallery = $("#product_gallery");
$_gallery.attr({"min-width": "{{ product.gallery.width }}px", "min-height": "{{ product.gallery.height }}px"});
$_gallery.etalage({
show_icon: false,
...
autoplay: false,
show_hint: false,
hide_cursor: false,{% if gallery_type == "click" %}show_lens : false,
zoom_element : '#zoom',
magnifier_opacity : 1,
{% elseif gallery_type == "zoom_inner" %}
show_lens : false,{% endif %}click_callback: function (image_anchor, instance_id, element) {
var images = [];
$.each($("a[rel='gallery']"), function (x, value) {
images.push({
type: "image",
caption: "",
thumb: $("img", $(value)).eq(0).attr("src"),
src: $(value).attr("href")
});
});
